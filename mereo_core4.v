...
Module Type Mereology (dolce:DOLCE_root).
Import dolce. 
...
Lemma ProtoT2 : forall (p q:Prop)(F:Morphism), (F p ≡≡ F q) -> (p ≡≡ q).
Proof.
intros a b F H.
apply (compat F).
assumption.
...
Parameter D7                  : forall a b, strong_equality a b ≡≡ ((exists G, G ε a) /\ 
...
Parameter D9                  : forall a b, strong_inclusion a b ≡≡ ((exists G, G ε a) /\ 
                                                                      forall G, G ε a -> G ε b).  (* D9 *)
...
Lemma OntoT14 : forall A, A ε A -> exists b, A ε b.
Proof.
intros A H.
exists A.
assumption.
...
Proof.
intros.
split;[intro H;rewrite H;reflexivity | intro H;apply compat_N;assumption].
Qed.
...
(* Polymorphic functor for rewriting mereological functors, e.g., pt, el and Kl *)
Parameter extens_functor  : forall A (f:N->N), Morphism_N (Psi epsilon A f).

Lemma MereoT0 : forall A B, A ε pt B -> B ε B.
Proof.
intros A B H.
apply asymmetric_Part in H.
apply D15 in H.
decompose [and] H.
assumption.
Qed.
...
Lemma MereoT16 : forall (A:N)(B:N)(C:N)(Phi:N->N), Morphism_N (Psi epsilon A Phi)-> ((Psi epsilon A Phi) B /\ singular_equality B C -> (Psi epsilon A Phi) C).
Proof.
intros A B C Phi H H'.
destruct H' as [H1 H2].
apply OntoT29 in H2.
...
Lemma MereoT26 : forall A a, Morphism_N (Psi' epsilon Kl Kl a) -> A ε Kl a -> A ε Kl (Kl a).
Proof.
intros A a H H0.
assert (H1:=H0).
assert (H2:=H0).
...
apply OntoT29 in H5;apply (N_rewrite (Psi' epsilon Kl Kl a) H) in H5;
unfold Psi' in H5;rewrite H5 in H3;assumption.
apply (extens_functor B Kl).
Qed.

Lemma MereoT27 : forall A a, Morphism_N (Psi'' epsilon Kl a) -> A ε Kl (Kl a) -> A ε Kl a.
Proof.
intros A a G H0.
assert (H1:=H0).
assert (H2:=H0).
...
apply OntoT29 in H5.
apply (N_rewrite (Psi'' epsilon Kl a) G) in H5.
unfold Psi'' in H5.
rewrite H5.
assumption.
apply (extens_functor A Kl).
Qed.
...
apply MereoT24 in H4.
assert (H11: G ε el B /\ singular_equality B (Kl a)).
split; assumption.
apply MereoT18 in H11.
...
Lemma MereoT40 : forall A B, (A ε el B) ≡≡ exists a, B ε (Kl a) /\ A ε a.
Proof.
intros A B.
split; [intro H1;assert (H2:=H1);apply klass_existence in H1;exists (el B);
split; [destruct H1;generalize x H;firstorder | assumption ] | 
intro; destruct H as [a H2]; destruct H2 as [H2 H3];
apply MereoT3 with (A:=B)(a:=a)(B:=A) in H2; assumption ].
Qed.

Lemma MereoT41 : forall A a, A ε (Kl a) -> A ε (coll a).
Proof.
intros A a H1.
apply MD2 in H1.
decompose [and] H1.
...
intros A B H1.
apply MD4.
split;[apply OntoT5 in H1;exact H1 |intros C H3;apply MD11;apply MD11 in H1;apply MD11 in H3;
      apply (Transitive_Element_of C B A);split; [assumption| assumption]].
Qed.
